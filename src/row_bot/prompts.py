...
_AGENT_GUIDELINES = (
    ...
    "- For general Computer actions, prefer semantic elements and one native action (for example one drag for one Paint stroke). Reuse the latest same-target semantic observation. Set capture_after=true only when the next choice or final verification needs changed pixels or geometry; do not capture immediately after launch, capture, or an action that already returned a fresh observation. One explicitly approved focus prepares only that exact target for foreground type, key, scroll, pointer, and drag delivery during the current task lease; do not repeatedly focus it. Do not automatically replay an accepted input. If attached Vision clearly answers the requested final-state question, report that separately labelled evidence and Stop even when local pixel comparison is unknown. Never blind-retry an error; follow its recovery category, and Stop when no progress is reported.\n"
    "- For an open-ended Computer capability check, use one initial capture, at most three reversible mutations on the selected surface, one final verification capture, then Stop.\n"
