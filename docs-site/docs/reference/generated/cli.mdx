...
<!-- inventory-hash: 280541553fc0b83e -->
...
| --- | --- | --- | --- | --- |
| row-bot | --browser |  |  | `src/row_bot/launcher.py:3447` |
| row-bot | --host | Host/interface for the Row-Bot local server |  | `src/row_bot/launcher.py:3495` |
| row-bot | --native |  |  | `src/row_bot/launcher.py:3452` |
| row-bot | --no-ollama | Do not try to auto-start Ollama |  | `src/row_bot/launcher.py:3472` |
| row-bot | --no-open | Do not open a browser or native window |  | `src/row_bot/launcher.py:3466` |
| row-bot | --no-splash | Skip the launcher splash screen |  | `src/row_bot/launcher.py:3469` |
| row-bot | --no-tray | Run without a system tray icon |  | `src/row_bot/launcher.py:3460` |
| row-bot | --port |  | _PORT | `src/row_bot/launcher.py:3492` |
| row-bot | --reset-db | Back up known local SQLite DBs before launch |  | `src/row_bot/launcher.py:3480` |
| row-bot | --reset-tasks-db | Back up and recreate tasks.db before launch |  | `src/row_bot/launcher.py:3475` |
| row-bot | --restore-data | Restore known SQLite DBs from a recovery backup directory or latest backup |  | `src/row_bot/launcher.py:3485` |
| row-bot | --server | Run the server without tray integration |  | `src/row_bot/launcher.py:3463` |
| row-bot | --tray | Force the system tray launcher |  | `src/row_bot/launcher.py:3457` |
...
| row-bot plugin doctor | plugin |  |  | `src/row_bot/launcher.py:3444` |
| row-bot plugin link | path |  |  | `src/row_bot/launcher.py:3436` |
| row-bot plugin reload | plugin_id |  |  | `src/row_bot/launcher.py:3440` |
| row-bot plugin validate | path |  |  | `src/row_bot/launcher.py:3434` |
