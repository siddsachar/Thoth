...
<!-- inventory-hash: a4c4fcbb57bc71b9 -->
...
| --- | --- | --- | --- | --- |
| row-bot | --browser |  |  | `src/row_bot/launcher.py:3453` |
| row-bot | --host | Host/interface for the Row-Bot local server |  | `src/row_bot/launcher.py:3501` |
| row-bot | --native |  |  | `src/row_bot/launcher.py:3458` |
| row-bot | --no-ollama | Do not try to auto-start Ollama |  | `src/row_bot/launcher.py:3478` |
| row-bot | --no-open | Do not open a browser or native window |  | `src/row_bot/launcher.py:3472` |
| row-bot | --no-splash | Skip the launcher splash screen |  | `src/row_bot/launcher.py:3475` |
| row-bot | --no-tray | Run without a system tray icon |  | `src/row_bot/launcher.py:3466` |
| row-bot | --port |  | _PORT | `src/row_bot/launcher.py:3498` |
| row-bot | --reset-db | Back up known local SQLite DBs before launch |  | `src/row_bot/launcher.py:3486` |
| row-bot | --reset-tasks-db | Back up and recreate tasks.db before launch |  | `src/row_bot/launcher.py:3481` |
| row-bot | --restore-data | Restore known SQLite DBs from a recovery backup directory or latest backup |  | `src/row_bot/launcher.py:3491` |
| row-bot | --server | Run the server without tray integration |  | `src/row_bot/launcher.py:3469` |
| row-bot | --tray | Force the system tray launcher |  | `src/row_bot/launcher.py:3463` |
...
| row-bot plugin doctor | plugin |  |  | `src/row_bot/launcher.py:3450` |
| row-bot plugin link | path |  |  | `src/row_bot/launcher.py:3442` |
| row-bot plugin reload | plugin_id |  |  | `src/row_bot/launcher.py:3446` |
| row-bot plugin validate | path |  |  | `src/row_bot/launcher.py:3440` |
