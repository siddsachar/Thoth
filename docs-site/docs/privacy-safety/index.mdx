...
See [Remote Access And Server Mode](/docs/operations/remote-access) for profiles, lifetimes, revocation, Tailscale ownership, server mode, Docker, and recovery.
...
- Keep Remote Access off until another trusted device needs it; use the narrowest route and profile.
