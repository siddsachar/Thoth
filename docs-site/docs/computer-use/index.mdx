...
4. Choose **Continue** only if you accept the disclosure. Row-Bot downloads the pinned Cua Driver 0.19.3 full archive for your platform, verifies its SHA-256, and extracts it into Row-Bot's private data directory. Upgrading from the older reviewed driver requires accepting the expanded version-2 telemetry notice again.
