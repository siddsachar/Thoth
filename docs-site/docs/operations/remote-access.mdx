...
description: "Connect trusted computers and companions through Tailscale, LAN, SSH, Docker, or an HTTPS reverse proxy while preserving Row-Bot authentication."
---
...
- A connected device receives only the capabilities of its `computer` or `companion` profile.
...
Open **Settings → System → Remote Access** from an authorized computer session.

1. Choose how the other device will reach this Row-Bot instance.
2. Check or enable that route.
3. Select **Invite a device**.
4. Choose the device profile, session lifetime, and one of the currently reachable connection routes.
...
<Screenshot id="remote-access-invitation" alt="Row-Bot device invitation dialog." caption="An invitation binds a profile, lifetime, and current browser-facing origin. It expires after 10 minutes and works once." />

## Invitations, Sessions, And Profiles
...
| Another computer | The complete owner UI, including owner-only settings and administration. |
| Phone or tablet | The restricted companion UI for Chat, Activity, workflows, Knowledge, and phone-safe settings. Screen size alone never grants this profile. |
...
Follow the complete [Docker deployment guide](https://github.com/siddsachar/row-bot/blob/main/deploy/docker/README.md) for:
