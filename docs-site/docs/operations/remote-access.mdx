...
<Screenshot id="settings-remote-access" alt="Row-Bot Remote Access settings." caption="Remote Access keeps connection routes separate from connected-device authorization. Tailscale is recommended; LAN remains an explicit, unencrypted option." />
...
1. Choose how the other device will reach this Row-Bot instance.
2. Check or enable that route.
3. Select **Invite a device**.
4. Choose the initial desktop or compact layout, session lifetime, and one of the currently reachable connection routes. For an address you have already configured, expand **Use another configured address** and enter its exact browser-facing HTTP or HTTPS origin instead.
5. Create the invitation and open or scan it on the other device.
...
The advanced address option only creates an origin-bound invitation. It does not configure or verify Cloudflare, DNS, TLS, proxy trust, firewall rules, or Row-Bot's listen address. Operator-managed Cloudflare and other reverse-proxy routes still require the existing public URL, allowed host, and exact trusted proxy configuration.
...
- **`unexpected_host`**: the browser-facing host is missing from `ROW_BOT_ALLOWED_HOSTS`, or the proxy forwarded a different host.
