...
- **Model catalog and picker clarity** — installed local Ollama chat models now appear in Settings → Models even when their family is not yet in Thoth's curated tool/vision capability lists, ChatGPT / Codex Vision pins keep their Vision capability during Quick Choice refreshes, and Brain and Vision pickers make it clear that catalog rows must be pinned before they appear as everyday choices.
- **Vision and model-setting hardening** — `thoth_update_setting` now validates Brain and Vision model changes against Quick Choices, installed local Ollama models, and provider catalog rows before saving, exposes an explicit `vision_model` setting, and rejects invented or unavailable model names with actionable guidance.
...
- **Linux packaged startup resilience** — packaged Linux launches now fail with startup log tails, child-process exit details, configurable `THOTH_STARTUP_TIMEOUT`, and targeted hints for native OpenCV/FAISS/NumPy dependency failures, while camera and screenshot capture degrade gracefully if OpenCV/MSS cannot import instead of blocking the app from starting.
...
- **Focused reliability coverage** — new startup hardening tests cover broken TorchCodec detection, Linux native dependency recovery hints, launcher log-tail diagnostics, Windows installer embedded-Python replacement, app import smoke, Settings → Models catalog bounds and picker guidance, status-tool model validation, safe timer cleanup, and installed Linux launcher symlink/default invocation resolution.
