...
- **Focused reliability coverage** — new startup hardening tests cover broken TorchCodec detection, launcher recovery hints, Windows installer embedded-Python replacement, app import smoke, Settings → Models catalog bounds and picker guidance, status-tool model validation, safe timer cleanup, and installed Linux launcher symlink resolution.
