...
- **Semantic editing contract** — tokenless `type` retains current-caret/selection insertion and rejects horizontal-tab structured payloads before approval or mutation. Token-bound `type` fresh-rematches one exact enabled writable text/search control without requiring selected state, forwards the fresh token to Cua's focus-and-insert transaction, tries background first, and permits one foreground driver call only after an explicit pre-dispatch refusal; it has no preliminary focus/click or replay fallback. `replace_text` calls reviewed Cua `set_value` once for a current exact editable token
- **Selective observation loop** — every input action requires current scope, policy, target, and element validation; dispatch, driver verdict, exact semantic postcondition, visual observation, action outcome, and generation completion stay distinct. Native exact value readback may verify unchanged pixels, while web echo with an unverifiable verdict and Catalyst-null evidence remain unresolved. Private replay state clears at lifecycle boundaries; a separate bounded value-free completion ledger survives for and is consumed by the final gate. Advisory Vision, Stop, cancel, or takeover cannot create success
