...
def test_buddy_overlay_uses_direct_accessible_actions_without_an_ellipsis_menu():
    buddy_ui_src = _read("src/row_bot/ui/buddy.py")
    ...
    assert 'ui.button(icon="unfold_more", on_click=_toggle_collapse)' in overlay_section
    ...
    collapsed_rule = buddy_ui_src.split(
        ".row-bot-buddy-overlay-page.row-bot-buddy-overlay-collapsed ",
        1,
    )[1].split("}", 1)[0]
    assert ".row-bot-buddy-overlay-body" in collapsed_rule
    assert "row-bot-buddy-overlay-actions" not in collapsed_rule
